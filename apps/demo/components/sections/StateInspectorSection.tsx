...
function getStatusColor(status: WebcamPhase): "success" | "info" | "error" | "default" {
  switch (status) {
    case "live":
      return "success";
    case "requesting":
  ...
  return (
    <Stack direction='column' spacing={3}>
      ...
      <ExampleCard title='웹캠 (상태 감지 중)'>
        <Webcam onStateChange={setSnapshot} visibleFlipButton visibleCameraDirectionButton />
      </ExampleCard>
      ...
      <Card variant='outlined'>
        ...
        <CardContent>
          {snapshot === null ? (
            <Typography color='text.secondary'>웹캠을 시작하면 상태가 여기에 표시됩니다</Typography>
          ) : (
            <Stack direction='column' spacing={1.5}>
              <Stack direction='row' spacing={1} sx={{ alignItems: "center" }}>
                ...
                <Chip label={snapshot.phase} color={getStatusColor(snapshot.phase)} size='small' />
              </Stack>
