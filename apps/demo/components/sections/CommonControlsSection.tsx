    ...
    <Stack direction='column' spacing={3}>
      ...
      <ExampleCard title='라이브 예제'>
        ...
        <Stack direction='row' spacing={1} sx={{ mt: 1.5, flexWrap: "wrap" }}>
          <Chip
            label={`facingMode: ${String(options.facingMode ?? "default")}`}
            size='small'
            variant='outlined'
          />
          <Chip
            label={`aspectRatio: ${String(options.aspectRatio ?? "auto")}`}
            size='small'
            variant='outlined'
          />
        </Stack>
        ...
        <Stack spacing={2}>
          ...
          <Stack direction='row' spacing={1} sx={{ flexWrap: "wrap" }}>
            ...
            <Button
              variant='outlined'
              size='small'
              onClick={() => setDefaultDemoKey((prev) => prev + 1)}
            >
              기본값 다시 적용
            </Button>
          </Stack>
          ...
          <Typography variant='body2'>
            현재 초기값 프리셋:{" "}
            {defaultPreset === "portrait" ? "세로 3:4 / environment" : "정사각형 1:1 / user"}
          </Typography>
        </Stack>
      </ExampleCard>
      ...
      <ExampleCard title='읽기 전용 controlled webcamOptions'>
        <Stack spacing={2}>
          ...
          <Stack direction='row' spacing={1} sx={{ flexWrap: "wrap" }}>
            ...
            <Chip
              label={`locked facingMode: ${lockedFacingMode}`}
              size='small'
              variant='outlined'
            />
          </Stack>
